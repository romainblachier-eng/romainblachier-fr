...
- **Pour vérifier la chaîne de production complète** (vérifié le 2026-08-25) : `tinacms build --skip-cloud-checks` ne contrôle pas la validité des identifiants, seulement leur présence. Il suffit donc de valeurs factices — `TINA_PUBLIC_CLIENT_ID=00000000-0000-0000-0000-000000000000 TINA_TOKEN=dummy npm run build` — pour faire tourner `tinacms build && astro build` de bout en bout et valider `tina/config.ts`. Les fichiers produits (`tina/__generated__/`, `public/admin/index.html`) sont ignorés par git, rien ne fuite.
- Corollaire : un jeton TinaCloud absent, expiré ou invalide **ne peut pas** faire échouer le build de production. Si le site ne se met plus à jour alors que les PR sont mergées, la cause n'est pas Tina — regarder du côté Netlify (publication automatique arrêtée, déploiement verrouillé, projet servant le domaine, branche de production).
- **Pour vérifier la chaîne de production complète** (vérifié le 2026-08-25) : `tinacms build --skip-cloud-checks` ne contrôle pas la validité des identifiants, seulement leur présence. Il suffit donc de valeurs factices — `TINA_PUBLIC_CLIENT_ID=00000000-0000-0000-0000-000000000000 TINA_TOKEN=dummy npm run build` — pour faire tourner `tinacms build && astro build` de bout en bout et valider `tina/config.ts`. Les fichiers produits (`tina/__generated__/`, `public/admin/index.html`) sont ignorés par git, rien ne fuite.
- Corollaire : un jeton TinaCloud absent, expiré ou invalide **ne peut pas** faire échouer le build de production. Si le site ne se met plus à jour alors que les PR sont mergées, la cause n'est pas Tina — regarder du côté Netlify (publication automatique arrêtée, déploiement verrouillé, projet servant le domaine, branche de production).
